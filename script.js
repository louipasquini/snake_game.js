...
const defineFoodPosition = () => {
    let newX = Math.floor(Math.random() * 60) * 10
    let newY = Math.floor(Math.random() * 60) * 10 
    while (snake.find((position) => position.x == newX && position.y == newY)) {
        newX = Math.floor(Math.random() * 60) * 10
        newY = Math.floor(Math.random() * 60) * 10 
    }
    food.x = newX
    food.y = newY
    if (direction == 'right') {
